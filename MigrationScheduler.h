...
class Board;
class Task;
vector <Task> running_app;
vector <Task> migrated_app;
vector <Board> boards_top;
...
class MigrationScheduler{
  ...
  public:
     MigrationScheduler(unsigned nb_of_boards);
     void _init_boards();
     unsigned get_available_core();
     Board get_suitable_board(unsigned memory);
     vector<Task> sorted(vector<Task> running_app, unsigned key);
     unsigned migrate();
